...
/**
 * Generate Qwen Code TOML Command Files from Markdown Commands
 *
 * This script converts all PRPROMPTS command .md files to Qwen Code .toml format
...
function generateTomlFile(category, commandName) {
  const mdPath = path.join(__dirname, '..', '.qwen', 'commands', category, `${commandName}.md`);
...
function main() {
  console.log('🚀 Generating Qwen Code TOML Command Files...\n');
  ...
  for (const [category, commands] of Object.entries(COMMANDS)) {
    ...
    const outputDir = path.join(__dirname, '..', '.qwen', 'commands', category);
  ...
  console.log('   .qwen/commands/prd/*.toml (6 files)');
  console.log('   .qwen/commands/planning/*.toml (4 files)');
  console.log('   .qwen/commands/prprompts/*.toml (5 files)');
  console.log('   .qwen/commands/automation/*.toml (6 files)');
  console.log('');
  ...
  console.log('   1. Copy to user config: cp -r .qwen/commands/* ~/.qwen/commands/');
