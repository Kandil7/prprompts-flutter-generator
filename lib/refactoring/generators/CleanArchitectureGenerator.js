...
const { toFeatureDirName, toFileName, toSnakeCase, toPascalCase } = require('./utils/namingConventions');
...
class CleanArchitectureGenerator {
  ...
  async generateMultiple(components) {
    ...
    for (const component of components) {
      try {
        ...
        // Collect routes
        if (component.navigation && component.navigation.routes.length > 0) {
          allRoutes.push(...component.navigation.routes.map(r => ({
            ...r,
            feature: component.feature || component.name
          })));
        }
      } catch (error) {
    ...
    // Generate AppRouter if routes found
    if (allRoutes.length > 0) {
      await this._generateAppRouter(allRoutes);
    }
  ...
  /**
   * Generate AppRouter using go_router
   * @param {Array} routes
   * @private
   */
  async _generateAppRouter(routes) {
    logger.info('Generating AppRouter...');
    ...
    // Add imports for pages
    const uniqueFeatures = [...new Set(routes.map(r => r.feature))];
    uniqueFeatures.forEach(feature => {
      const featureDir = toFeatureDirName(feature);
      const pageName = toFileName(feature + 'Page');
      imports.push(`import 'features/${featureDir}/presentation/pages/${pageName}.dart';`);
    });

    let code = imports.join('\n') + '\n\n';

    code += '/// App Router configuration using go_router\n';
    code += 'class AppRouter {\n';
    code += '  static final router = GoRouter(\n';
    code += '    initialLocation: \'/\',\n';
    code += '    routes: [\n';

    routes.forEach(route => {
      const path = route.path === '/' ? '/' : (route.path.startsWith('/') ? route.path : `/${route.path}`);
      const pageName = toPascalCase(route.feature || 'Home') + 'Page';

      code += '      GoRoute(\n';
      code += `        path: '${path}',\n`;
      code += `        builder: (context, state) => const ${pageName}(),\n`;
      code += '      ),\n';
    });
    ...
    await this.fileHandler.ensureDir('lib/core/router');
    await this.fileHandler.writeFile('lib/core/router/app_router.dart', code);

    logger.success('Generated AppRouter');
  }
}
