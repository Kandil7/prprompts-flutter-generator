...
class WidgetGenerator {
  ...
  _parseStyleObject(styleString) {
    try {
      ...
      for (const part of parts) {
        ...
        // Convert numeric strings to numbers (but not if they have units like 'px', 'rem', '%')
        if (/^-?\d+(\.\d+)?$/.test(value)) {
          value = parseFloat(value);
        }
        // Keep string values with units as strings (will be converted by styleConverter)
        // This includes: '100px', '2rem', '50%', etc.
      ...
      logger.warn('Failed to parse style object', {
        styleString,
        error: error.message
      });
      return null;
  ...
  _smartSplit(str, delimiter) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      // Track nesting depth (objects, arrays)
      if (!inString) {
        if (char === '{' || char === '[') depth++;
        if (char === '}' || char === ']') depth--;

        // Split at delimiter only when not nested
