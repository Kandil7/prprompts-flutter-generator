...
class ReactParser {
  ...
  _extractComponentInfo(ast, filePath) {
    ...
    traverse(ast, {
      ...
      ExportDeclaration: (path) => {
        ...
        if (exportInfo.type === 'default') {
          ...
          if (astHelpers.isReactComponent(exportInfo.declaration)) {
    ...
    if (!componentNode || !componentName) {
      // Fallback: use filename if we found a component node but no name
      if (componentNode && filePath) {
        const baseName = path.basename(filePath, path.extname(filePath));
        // Ensure PascalCase
        componentName = baseName.charAt(0).toUpperCase() + baseName.slice(1);
        logger.debug(`Inferred component name '${componentName}' from filename`);
      }
    ...
    const hasReactImport = imports.some(i => i.source.toLowerCase() === 'react');
    if (!hasReactImport) {
    ...
    children.forEach(child => {
      // Skip HTML tags (lowercase) - handled by _getJSXTagName check
      // Check if imported or defined locally
      const isImported = imports.some(i => i.specifiers.some(s => s.local === child));
      const isDefined = definedComponents.has(child);

      if (!isImported && !isDefined) {
        warnings.push(`Missing import for component: ${child}`);
      }
    });
