...
const logger = createModuleLogger('RefactorCommand');
...
class RefactorCommand {
  ...
  * @param {string} reactSource - React source directory (optional if set in constructor)
  * @param {string} flutterTarget - Flutter target directory (optional if set in constructor)
  * @param {Object} options - Conversion options (optional if set in constructor)
  */
  async execute(reactSource, flutterTarget, options = {}) {
    try {
      // Allow parameters to override constructor options
      if (reactSource) this.options.reactSource = reactSource;
      if (flutterTarget) this.options.flutterTarget = flutterTarget;
  ...
  async validateInputs() {
    ...
    try {
      ...
      // Re-throw with original message if it mentions "not a directory"
    ...
    if (this.options.ai !== 'none') {
      const validProviders = ['claude', 'qwen', 'gemini', 'mock', 'none'];
  ...
  async parseReactFiles() {
    ...
    for (const file of reactFiles) {
      try {
        ...
        const component = parser.parse(content, file);
    ...
    if (reactFiles.length === 0) {
      this.stats.warnings.push({
        type: 'no_files_found',
        message: 'No React files found in source directory'
      });
    }
  ...
  async generateFlutterCode(components) {
    ...
    const generator = this.options.architecture === 'clean'
      ? new CleanArchitectureGenerator({
        outputPath: this.options.flutterTarget,
        config: {
    ...
    for (const component of components) {
      try {
        const files = await generator.generate(component);
  ...
  async enhanceWithAI(generatedFiles) {
    ...
    // Create AI client and services using factory functions
    const aiClient = createAIClient(this.options.ai);
    const { codeEnhancer, widgetOptimizer, testGenerator } = createAIServices(aiClient, logger);
    ...
    for (const [filePath, content] of Object.entries(generatedFiles)) {
      try {
        ...
        const enhanced = await codeEnhancer.enhance(content, { filePath });

        // Optimize widgets
        if (filePath.includes('/widgets/') || filePath.includes('/pages/')) {
          const optimized = await widgetOptimizer.optimize(null, enhanced);
          generatedFiles[filePath] = optimized.code || optimized;
        } else {
          generatedFiles[filePath] = enhanced.enhancedCode || enhanced;
        }

        // Generate tests
        if (!filePath.includes('_test.dart')) {
          const testPath = filePath.replace(/\.dart$/, '_test.dart').replace('/lib/', '/test/');
          const testCode = await testGenerator.generateTests(null, generatedFiles[filePath]);
          if (testCode && testCode.code) {
            generatedFiles[testPath] = testCode.code;
  ...
  async validateOutput() {
    ...
    const results = await orchestrator.validateProject(this.options.flutterTarget);
  ...
  colorize(text, color) {
    ...
    // Only add reset code if a valid color was used
    if (colors[color]) {
